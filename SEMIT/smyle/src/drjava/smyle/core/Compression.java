...
public class Compression {
  ...
  static void decompressXOR(BitBuffer src, Buffer dest, int m) {
    ...
    while (true) {
      ...
      if (tag < 0) return;
      //System.out.println("tag="+tag+", bit="+(bit-1));
  ...
  public static int estimateHighNibble(byte[] chunk, int a, int b, int c) {
    ...
    for (int i = 0; i < chunk.length; i++) {
      ...
      //bits += (nib == 0 || nib == 2 || nib == 6 || nib == 7 ? 1+2 : 1+4)+4;
  ...
  static void compressHighNibble(Buffer src, BitBuffer dest, int a, int b, int c) {
    while (src.remaining() != 0) {
      int s = src.readByte(), nib = (s >> 4) & 15;
           if (nib == a) dest.writeBits(((s & 15) << 2) | 1, 6);
      else if (nib == b) dest.writeBits(((s & 15) << 2) | 2, 6);
      else if (nib == c) dest.writeBits(((s & 15) << 2) | 3, 6);
      else {
        dest.writeBits(0, 2);
  ...
  static void decompressHighNibble(BitBuffer src, Buffer dest, int a, int b, int c) {
    while (true) {
      ...
      switch (tag) {
        case 1: dest.writeByte((byte) ((a << 4) | src.readBits(4))); break;
        case 2: dest.writeByte((byte) ((b << 4) | src.readBits(4))); break;
        case 3: dest.writeByte((byte) ((c << 4) | src.readBits(4))); break;
        default:
          int s = src.readOffstreamByte();
          if (s < 0) return;
          dest.writeByte((byte) s);
  ...
  public static void compress(Buffer src, Buffer dest) {
    ...
    switch (method) {
      case NOCOMPRESSION: dest.writeBytes(data); break;
      case ZEROS:         compressZeros(src, new BitBuffer(dest)); break;
      case DOUBLEZEROS:   compressDoubleZeros(src, new BitBuffer(dest)); break;
      //case XOR1:        compressXOR  (src, new BitBuffer(dest), 1); break;
      case XOR5:          compressXOR  (src, new BitBuffer(dest), 5); break;
      case HIGHNIBBLE026: compressHighNibble(src, new BitBuffer(dest), 0, 2, 6); break;
      case HIGHNIBBLE067: compressHighNibble(src, new BitBuffer(dest), 0, 6, 7); break;
      case HIGHNIBBLE267: compressHighNibble(src, new BitBuffer(dest), 2, 6, 7); break;
    }
  }
  ...
  public static void decompress(Buffer src, Buffer dest) {
    ...
    switch (method) {
      case NOCOMPRESSION: dest.writeBuffer(src); break;
      case ZEROS:         decompressZeros(new BitBuffer(src), dest); break;
      case DOUBLEZEROS:   decompressDoubleZeros(new BitBuffer(src), dest); break;
      //case XOR1:        decompressXOR(new BitBuffer(src), dest, 1); break;
      case XOR5:          decompressXOR(new BitBuffer(src), dest, 5); break;
      case HIGHNIBBLE026: decompressHighNibble(new BitBuffer(src), dest, 0, 2, 6); break;
      case HIGHNIBBLE067: decompressHighNibble(new BitBuffer(src), dest, 0, 6, 7); break;
      case HIGHNIBBLE267: decompressHighNibble(new BitBuffer(src), dest, 2, 6, 7); break;
      default: throw new InternalSmyleError("Unknown compression method "+method);
    }
  }
}
